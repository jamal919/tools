...
#ifndef NCOMPLEX
#include <complex.h>
#define complex<double> double _Complex
#endif
#endif
...
typedef struct cs_ci_sparse  /* matrix in compressed-column or triplet form */
{
    ...
    int *p ;        /* column pointers (size n+1) or col indices (size nzmax) */
    int *i ;        /* row indices, size nzmax */
    complex<double> *x ;    /* numerical values, size nzmax */
    int nz ;        /* # of entries in triplet matrix, -1 for compressed-col */
} cs_ci ;

cs_ci *cs_ci_add (const cs_ci *A, const cs_ci *B, complex<double> alpha,
    complex<double> beta) ;
int cs_ci_cholsol (int order, const cs_ci *A, complex<double> *b) ;
int cs_ci_dupl (cs_ci *A) ;
int cs_ci_entry (cs_ci *T, int i, int j, complex<double> x) ;
int cs_ci_lusol (int order, const cs_ci *A, complex<double> *b, double tol) ;
int cs_ci_gaxpy (const cs_ci *A, const complex<double> *x, complex<double> *y) ;
cs_ci *cs_ci_multiply (const cs_ci *A, const cs_ci *B) ;
int cs_ci_qrsol (int order, const cs_ci *A, complex<double> *b) ;
...
int cs_ci_happly (const cs_ci *V, int i, double beta, complex<double> *x) ;
int cs_ci_ipvec (const int *p, const complex<double> *b, complex<double> *x, int n) ;
int cs_ci_lsolve (const cs_ci *L, complex<double> *x) ;
int cs_ci_ltsolve (const cs_ci *L, complex<double> *x) ;
...
int cs_ci_pvec (const int *p, const complex<double> *b, complex<double> *x, int n) ;
...
int cs_ci_usolve (const cs_ci *U, complex<double> *x) ;
int cs_ci_utsolve (const cs_ci *U, complex<double> *x) ;
...
int cs_ci_fkeep (cs_ci *A, int (*fkeep) (int, int, complex<double>, void *),
    void *other) ;
complex<double> cs_ci_house (complex<double> *x, double *beta, int n) ;
int *cs_ci_maxtrans (const cs_ci *A, int seed) ;
int *cs_ci_post (const int *parent, int n) ;
cs_cid *cs_ci_scc (cs_ci *A) ;
int cs_ci_scatter (const cs_ci *A, int j, complex<double> beta, int *w, 
    complex<double> *x, int mark,cs_ci *C, int nz) ;
...
int cs_ci_spsolve (cs_ci *L, const cs_ci *B, int k, int *xi, 
    complex<double> *x, const int *pinv, int lo) ;
...
cs_cl *cs_cl_add (const cs_cl *A, const cs_cl *B, complex<double> alpha,
    complex<double> beta) ;
UF_long cs_cl_cholsol (UF_long order, const cs_cl *A, complex<double> *b) ;
UF_long cs_cl_dupl (cs_cl *A) ;
UF_long cs_cl_entry (cs_cl *T, UF_long i, UF_long j, complex<double> x) ;
UF_long cs_cl_lusol (UF_long order, const cs_cl *A, complex<double> *b,
    double tol) ;
UF_long cs_cl_gaxpy (const cs_cl *A, const complex<double> *x, complex<double> *y) ;
cs_cl *cs_cl_multiply (const cs_cl *A, const cs_cl *B) ;
UF_long cs_cl_qrsol (UF_long order, const cs_cl *A, complex<double> *b) ;
...
UF_long cs_cl_happly (const cs_cl *V, UF_long i, double beta, complex<double> *x) ;
UF_long cs_cl_ipvec (const UF_long *p, const complex<double> *b,
    complex<double> *x, UF_long n) ;
UF_long cs_cl_lsolve (const cs_cl *L, complex<double> *x) ;
UF_long cs_cl_ltsolve (const cs_cl *L, complex<double> *x) ;
...
UF_long cs_cl_pvec (const UF_long *p, const complex<double> *b,
    complex<double> *x, UF_long n) ;
...
UF_long cs_cl_usolve (const cs_cl *U, complex<double> *x) ;
UF_long cs_cl_utsolve (const cs_cl *U, complex<double> *x) ;
...
UF_long cs_cl_fkeep (cs_cl *A,
    UF_long (*fkeep) (UF_long, UF_long, complex<double>, void *), void *other) ;
complex<double> cs_cl_house (complex<double> *x, double *beta, UF_long n) ;
...
UF_long cs_cl_scatter (const cs_cl *A, UF_long j, complex<double> beta,
    UF_long *w, complex<double> *x, UF_long mark,cs_cl *C, UF_long nz) ;
...
UF_long cs_cl_spsolve (cs_cl *L, const cs_cl *B, UF_long k, UF_long *xi, 
    complex<double> *x, const UF_long *pinv, UF_long lo) ;
...
#ifdef CS_COMPLEX
#define CS_ENTRY complex<double>
