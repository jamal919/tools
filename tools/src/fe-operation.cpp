...
{
  float *rbuf;
  float  dummy,rmask,*rbuffer[2],mask;
  ...
  FILE *file;
  FILE *out;
  char *meshfile=NULL,*sfile[2],*cfile=NULL,*sfmt_in=NULL,*sfmt_out=NULL,*cdl=0;
  ...
  while (n < argc) {
    ...
    switch (keyword[0]) {
      ...
      default:
        if(sfile[0]==NULL) {
          sfile[0]= strdup(argv[n]);
          printf("1st input file=%s\n",sfile[0]);
          n++;
          }
        else if(sfile[1]==NULL) {
          sfile[1]= strdup(argv[n]);
          printf("2nd input file=%s\n",sfile[0]);
          n++;
          }
        else {
          __OUT_BASE_LINE__("unknown option %s\n",keyword);
...
//   if(sfmt_out==NULL) sfmt_out= strdup("netcdf");
  ...
  if(sfmt_in==0) {
    if(strstr(sfile[0],".s2r") != NULL)    {
      fefmt=S2R;
      sfmt_in=strdup("s2r");
      }
    if(strstr(sfile[0],".s2c") != NULL)    {
      fefmt=S2C;
      sfmt_in=strdup("s2c");
      }
    if(strstr(sfile[0],".v2r") != NULL)    {
      fefmt=V2R;
      sfmt_in=strdup("v2r");
      }
    if(strstr(sfile[0],".v2c") != NULL)    {
      fefmt=V2C;
      sfmt_in=strdup("v2c");
      }
    if(strstr(sfile[0],".dat") != NULL)    {
      fefmt=S2R;
      sfmt_in=strdup("s2r");
      }
    }
...
/* *----------------------------------------------------------------------------
    ...
    if(strcmp(sfmt_in,"srf")==0) {
      fefmt=SRF;
      dim=S2R;
      translation=surfref_reference(sfile[0], mesh, &ncolumns);
      }
    }
    
  if(sfmt_out==NULL) sfmt_out= strdup(sfmt_in);
   
  if(strcmp(sfmt_out,"srf")==0) {
     strcpy(output,sfile[0]);
     root=strstr(output,".dat");
     sprintf(root,"%s",".mesh-array.dat");
     status=surfref_reorder(sfile[0], mesh, ncolumns, translation, output);
  ...
  switch (dim) {
    case S2R:
      rbuffer[0]=new float[nndes];
      rbuffer[1]=new float[nndes];
      for (k=0;k<ncolumns;k++) {
        status=loadr1(sfile[0], nndes, ncolumns, k, translation, rbuffer[0],&rmask,fefmt);
        if(status!=0) goto error;
        status=loadr1(sfile[1], nndes, ncolumns, k, translation, rbuffer[1],&rmask,fefmt);
        if(status!=0) goto error;
        for(int n=0;n<nndes;n++) {
          if(rbuffer[0][n]==mask) continue;
          if(rbuffer[1][n]==mask) {
            rbuffer[0][n]=mask;
            continue;
            }
         else {
            rbuffer[0][n]-=rbuffer[1][n];
            }
          }
        ...
        if(strcmp(sfmt_out,"s2r")==0) {
          ...
          status=quoddy_saver1(output, mesh.nvtxs,rbuffer[0],comments);
          }
          ...
          status   = poc_put_UG2D(output, mesh, variable.id, rbuffer[0]);
          variable.destroy();
          }
        }
      free(rbuffer);
    ...
    case V2R:
      ...
      status=quoddy_loadr2(sfile[0], nndes, rbufferx,rbuffery);
      if(strcmp(sfmt_out,"ascii")==0) {
        }
      else if(strcmp(sfmt_out,"netcdf")==0) {
        strcpy(output,sfile[0]);
    ...
    case S2C:
      ...
      status=quoddy_loadc1(sfile[0], nndes, cbuffer);
      if(strcmp(sfmt_out,"ascii")==0) {
        }
      else {
        }
    ...
    case V2C:
      ...
      status=quoddy_loadc2(sfile[0], nndes, cbufferx, cbuffery);
      if(strcmp(sfmt_out,"ascii")==0) {
        }
      else {
        }
