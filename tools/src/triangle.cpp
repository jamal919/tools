...

{
  ...
  memptr = new VOID[(unsigned int) size];
  if (memptr == (VOID *) NULL) {
    printf("Error:  Out of memory.\n");
    triexit(1);
  }
...

{
  ...
  for (j = 1; j < m->invertices; j++) {
    if ((sortarray[i][0] == sortarray[j][0])
      ...
      if (!b->quiet) {
        printf(
"Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored (%d %d).\n",
               sortarray[j][0], sortarray[j][1],i,j);
      }
