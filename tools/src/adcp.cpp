...
//      k=index_from_name(spectrum_t s,char *name)
  ...
  for(i=0;i<nmgr;i++) {
    mgr_t adcp;
    adcp.number=i;
    strcpy(adcp.name,"no_name");
    strcpy(adcp.origine,"undocumented");
    adcp.data=data;
    adcp.nwave=AnalysisList.n;
    adcp.loc.units=strdup("degrees");    sprintf(adcp.debut,"%2d/%2d/%4d",start.day,start.month,start.year);

    adcp.loc.lon=0.0;
    adcp.loc.lat=0.0;
    adcp.mindex=1;
    sprintf(adcp.debut,"%2d/%2d/%4d",start.day,start.month,start.year);
    sprintf(adcp.fin,"%2d/%2d/%4d",final.day,final.month,final.year);
    mgr.push_back(adcp);
    }
